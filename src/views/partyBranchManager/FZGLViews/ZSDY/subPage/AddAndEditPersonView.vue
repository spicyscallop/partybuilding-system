    ...
    <v-row>
      <v-col style="background-color: #f7f7f7; height: 90%;padding: 20px;border-radius: 20px;">
        ...
        <v-row>
          ...
          <v-col cols="6">
            <div style="margin-bottom: 5px;"><span>提出转正申请时间</span></div>
            <div>
              <el-date-picker v-model="form.applyFullTime" type="date" placeholder="yyyy-mm-dd" size="large"
                              style="width: 90%;"/>
            </div>
          </v-col>
        </v-row>
        ...
        <v-row>
          ...
          <v-col cols="6">
            <div style="margin-bottom: 5px;"><span>支部大会时间</span></div>
            <div>
              <el-date-picker v-model="form.formalMemberConfirmationTime" type="date" placeholder="yyyy-mm-dd" size="large"
              style="width: 90%;" />
            </div>
          </v-col>
        </v-row>
        ...
        <v-row>
          <v-col cols="6">
            ...
            <div>
              <el-date-picker v-model="form.developPartyTraining" type="date" placeholder="系统自动接入" size="large"
      ...
      if (this.isEdit) {
        this.$axios.post('/stage/update', submitData)
            .then(response => {
              ...
              this.$router.push({name: 'P_ZSDY'}); // 替换为您的列表页面路由名称
            })
            .catch(error => {
