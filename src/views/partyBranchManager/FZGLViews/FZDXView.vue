    ...
    <v-container fluid class="ma-0 fill-height bg-grey-lighten-3">
        <v-col class="d-flex fill-height flex-column">
            <v-row class="flex-shrink-1 flex-grow-0">
                <v-col>
                    <SubpageTitle text="发展对象阶段" icon="mdi-book-open-variant" @drawerToggle="$emit('drawerToggle')">
                    </SubpageTitle>
                </v-col>
            </v-row>
                            ...
                            <v-card-text>
                                <v-container>
                                    <v-row>
                                        ...
                                        <v-col cols="5" class="d-flex flex-row align-center">
                                            发展对象确定时间
                                            <VDatePicker v-model="editDialog.data.confirmTime"
                                        ...
                                        <v-col cols="5" class="d-flex flex-row align-center">
                                            发展对象备案时间
                                            <VDatePicker v-model="editDialog.data.recordTime"
                                                :popover="datePicker.clickPop">
                                                <template #default="{ inputValue, inputEvents }">
                                                    <v-text-field class="ml-3 date-picker"
                                                        prepend-inner-icon="mdi-calendar-month" variant="outlined"
                                        ...
                                        <v-col cols="5" class="d-flex flex-row align-center">
                                            党校培训班参与时间
                                            <VDatePicker v-model="editDialog.data.developPartyTraining"
                                                :popover="datePicker.clickPop">
                                                <template #default="{ inputValue, inputEvents }">
                                                    <v-text-field class="ml-3 date-picker"
                                                        prepend-inner-icon="mdi-calendar-month" variant="outlined"
                                        ...
                                        <v-col cols="5" class="d-flex flex-row align-center">
                                            入党介绍人
                                            <v-text-field v-model="editDialog.data.partySponsor" class="ml-3"
                                                variant="outlined" density="compact" hide-details="auto"
                                        ...
                                        <v-col cols="5" class="d-flex flex-row align-center">
                                            <VDatePicker v-model="editDialog.data.politicalReviewTime"
                                                :popover="datePicker.clickPop">
                                                <template #default="{ inputValue, inputEvents }">
                                                    <v-text-field class="ml-3 date-picker"
                                                        prepend-inner-icon="mdi-calendar-month" variant="outlined"
                                        ...
                                        <v-col cols="5" class="d-flex flex-row align-center">
                                            <VDatePicker v-model="editDialog.data.partyApplicationTime"
                                                :popover="datePicker.clickPop">
                                                <template #default="{ inputValue, inputEvents }">
                                                    <v-text-field class="ml-3 date-picker"
                                                        prepend-inner-icon="mdi-calendar-month" variant="outlined"
                                        ...
                                        <v-col cols="5" class="d-flex flex-row align-center">
                                            审查时间
                                            <VDatePicker v-model="editDialog.data.investigateTime"
                                                :popover="datePicker.clickPop">
                                                <template #default="{ inputValue, inputEvents }">
                                                    <v-text-field class="ml-3 date-picker"
                                                        prepend-inner-icon="mdi-calendar-month" variant="outlined"
                                        ...
                                        <v-col cols="5" class="d-flex flex-row align-center">
                                            预审时间
                                            <VDatePicker v-model="editDialog.data.preliminaryInvestigateTime"
                                                :popover="datePicker.clickPop">
                                                <template #default="{ inputValue, inputEvents }">
                                                    <v-text-field class="ml-3 date-picker"
                                                        prepend-inner-icon="mdi-calendar-month" variant="outlined"
                    ...
                    <v-sheet class="d-flex flex-grow-1" rounded="lg" max-width="100%">
                        <v-col class="d-flex flex-grow-1 flex-column">
                            <v-row class="flex-shrink-1 flex-grow-0 ma-0">
                                ...
                                <v-col class="d-flex justify-center">
                                    <v-label class="mr-2">发展对象确定时间</v-label>
                                    <VDatePicker v-model="query.confirmTime">
                                        <template #default="{ inputValue, inputEvents }">
                            ...
                            <v-row id="data-table" class="flex-grow-1 flex-shrink-1 fill-height"
                                ...
                                <v-col class="pa-0 fill-height" style="width: 0; flex: 1 1 0;">
                                    <v-table class="data-table-inner data-table-center fill-height" fixed-header>
                                        ...
                                        <tbody>
                                            <tr v-for="item in table" :key="item.userNumber">
                                                <td>{{ timestampToString(item.confirmTime) }}</td>
                                                <td>{{ timestampToString(item.recordTime) }}</td>
                                                <td>{{ item.partySponsor }}</td>
                                                <td>{{ timestampToString(item.developPartyTraining) }}</td>
                                                <td>{{ timestampToString(item.politicalReviewTime) }}</td>
                                                <td>{{ timestampToString(item.investigateTime) }}</td>
                                                <td>{{ timestampToString(item.preliminaryInvestigateTime) }}</td>
                                                <td>{{ timestampToString(item.partyApplicationTime) }}</td>
                                                <td>{{ item.recommendable }}</td>
                                            </tr>
                                        </tbody>
                                    </v-table>
                                </v-col>
...
export default {
    ...
    data() {
        return {
            ...
            event: null,
            stateMap: {
                "成为发展对象": [true, true, false, false, false, false, false, false],
                "确定入党介绍人": [false, false, false, true, false, false, false, false],
                "提交入党对象政治审查综合表": [false, false, false, false, true, false, false, false],
                "参加发展对象阶段党校培训班": [false, false, true, false, false, false, false, false],
                "资格审查": [false, false, false, false, false, false, true, false],
                "资格预审": [false, false, false, false, false, false, false, true],
                "提交入党志愿书": [false, false, false, false, false, true, false, false],
                "变更身份为预备党员": [false, false, false, false, false, false, false, false],
            },
            editDialog: {
                isOpen: false,
                userNumber: 0,
    ...
    methods: {
        ...
        getValidData() {
            ...
            ["confirmTime", "developPartyTraining", "partyApplicationTime", "politicalReviewTime", "investigateTime", "preliminaryInvestigateTime", "recordTime"].forEach((key) => {
        ...
        processQuery(skipVerify = false, fn = () => { }) {
            ...
            if (queryParam["confirmTime"])
                queryParam["confirmTime"] = dayjs(queryParam["confirmTime"]).format("YYYY-MM-DD");

            queryParam["pageNumber"] = this.page;
            queryParam["pageSize"] = this.pageSize.substring(0, 2);

            axios.get("/stages/develop", {
                params: queryParam
            }).then(response => {
                this.pages = response.data.data.pages;
                if (!this.pages) this.pages = 1;
                this.table = response.data.data.developStageVOList;
