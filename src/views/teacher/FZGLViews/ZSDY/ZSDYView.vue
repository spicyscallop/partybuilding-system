        ...
        <div
          ...
          <v-col cols="10">
            ...
            <input placeholder="    请输入姓名" v-model="queryItems.name"
                   style=" margin-left:10px;background-color: white;font-size: 12px;height: 25px; width: 10%;">
            <span style="margin-left: 30px;">正式党员确定时间</span>
...
export default {
  ...
  data() {
    return {
      ...
      colNames: ['学工号', '姓名', '《群众意见调查表》提交时间', '提出转正申请时间', '支部大会时间', '党委审批时间','思想汇报提交时间'],
      originColumns:[],
      columns: [
        {
          label: '学工号',
        ...
        {
          label: '支部大会时间',
          prop: 'formalMemberConfirmationTime',
          type: 'date'
        },
        {
          label: '思想汇报提交时间',
  ...
  methods: {
    ...
    goToAddPersonView() {
      this.$router.push({ name: 'EditPersonViewZSDYT' });
    },
    editRow() {
      ...
      this.$router.push({ name: 'EditPersonViewZSDYT', params: { id: this.selectedRows[0].id } });
    },
    deleteRow() {
      if (this.selectedRows.length === 0) {
