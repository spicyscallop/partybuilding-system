        ...
        <div
          ...
          <v-col cols="10">
            ...
            <input placeholder="    请输入姓名" v-model="queryItems.name"
                   style=" margin-left:10px;background-color: white;font-size: 12px;height: 25px; width: 10%;">
            <span style="margin-left: 30px;">发展对象确定时间</span>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          label: '《入党对象政治审查综合表》提交时间',
          prop: 'politicalReviewTime',
          type: 'date'
        },
        {
          label: '思想汇报提交时间',
  ...
  methods: {
    ...
    goToAddPersonView() {
      this.$router.push({ name: 'EditPersonViewFZDXT' });
    },
    editRow() {
      ...
      this.$router.push({ name: 'EditPersonViewFZDXT', params: { id: this.selectedRows[0].id } });
    },
    deleteRow() {
      if (this.selectedRows.length === 0) {
