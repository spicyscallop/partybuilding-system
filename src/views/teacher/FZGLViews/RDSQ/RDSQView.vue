<template>
          ...
          <div class="flex-grow-1 overflow-auto">
            ...
            >
              ...
              <el-table-column v-for="item in columns" v-show="false" :prop="item.prop" :label="item.label" :width="item.width || ''" align="center">
                ...
                <template v-slot="scope" v-if="item.type === 'isBoolean'">
                  {{ formatBoolean(scope.row[item.prop]) }}
                </template>
              </el-table-column>
            </el-table>
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      goToAddPersonView() {
        this.$router.push({ name: 'EditPersonViewRDSQT' });
      },
      editRow() {
        ...
        this.$router.push({ name: 'EditPersonViewRDSQT', params: { id: this.selectedRows[0].id } });
      },
      deleteRow() {
        if (this.selectedRows.length === 0) {
